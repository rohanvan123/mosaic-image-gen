...
class KDTree
{
  ...
  private:
     ...
     */
    KDTreeNode* buildNode(std::vector<Point<Dim>>& points, int l, int r, int d);
    Point<Dim> quickselect(std::vector<Point<Dim>>& points, int l, int r, int k, int d);
    int partition(std::vector<Point<Dim>>& points, int l, int r, int pivotIndex, int d);
    void swap(std::vector<Point<Dim>>& points, int b, int s);
    Point<Dim> nearestNeighbor(const Point<Dim>& query, KDTreeNode* curr, int d) const;
    bool sameDistance(const Point<Dim>& target, const Point<Dim>& currentBest, const Point<Dim>& potential) const;
};
